  ...
  try {
    ...
    if (error && error.code === "EROFS") {
      return NextResponse.json(
        {
          error:
            "Save failed on Vercel: the deployment filesystem is read-only. Use external storage (database/blob) for editable content."
        },
        { status: 500 }
      );
    }
    const message = error instanceof Error && error.message ? error.message : "Failed to save content";
